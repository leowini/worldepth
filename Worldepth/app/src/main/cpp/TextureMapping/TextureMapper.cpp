...
double TextureMapper::Tixi(int &x, int &y, int &t, std::vector<std::vector<float>> &completenessPatches,
    ...
    for (unsigned long k = 0; k < N; k++) {
        ...
        cv::projectPoints(Xi, rvec, pose(cv::Rect(3, 0, 1, 3)), intrinsicMatrix, distCoef, Xik);
...
double TextureMapper::Mixi(int &x, int &y, int &t) {
    ...
    for (int j = 0; j < N; j++) {
        ...
        std::vector<cv::Point2f> Xi;
        Xi.emplace_back(cv::Point2f(x, y));
        cv::projectPoints(Xi, rvec, pose(cv::Rect(3, 0, 1, 3)), intrinsicMatrix, distCoef, Xij);
...
void TextureMapper::convertToRaster(const cv::Point3f &vertexWorld, cv::Mat &cameraMatrix,
...
void TextureMapper::getRGBD() {
    ...
    for (unsigned int cam = 0; cam < TcwPoses.size(); cam++) {
        ...
        for (uint32_t i = 0; i < ntris; i++) {
            ...
            cv::Mat cameraMatrix = intrinsicMatrix * pose(cv::Rect(0, 0, 4, 3));
...
void TextureMapper::projectToSurface() {
    ...
    for (unsigned int cam = 0; cam < TcwPoses.size(); cam++) {
        ...
        cv::projectPoints(vertices, rvec, pose(cv::Rect(3, 0, 1, 3)), intrinsicMatrix, distCoef,
