...
void TextureMapper::write_ply_file(double *weights, double *acc_red, double *acc_grn, double *acc_blu) {
    ...
    for (int buff_ind = 0; buff_ind < vertices.size(); buff_ind++) {
        ...
        if (weights[buff_ind] != 0) // if 0, it has not found any valid projection on any camera
        {
            ...
            int r = (acc_red[buff_ind] / weights[buff_ind]) * 255.0;
            int g = (acc_grn[buff_ind] / weights[buff_ind]) * 255.0;
            int b = (acc_blu[buff_ind] / weights[buff_ind]) * 255.0;
            out.write(" ", sizeof(" "));
            out.write(reinterpret_cast<uint8_t*>(&r), sizeof(r));
            out.write(" ", sizeof(" "));
            out.write(reinterpret_cast<uint8_t*>(&g), sizeof(g));
            out.write(" ", sizeof(" "));
            out.write(reinterpret_cast<uint8_t*>(&b), sizeof(b));
        } else {
            int r = 0;
            int g = 0;
            int b = 0;
...
void write_ply_example(double *weights, double *acc_red, double *acc_grn, double *acc_blu)
{
    std::filebuf fb_binary;
    fb_binary.open(tempFilename + "-binary.ply", std::ios::out | std::ios::binary);
    std::ostream outstream_binary(&fb_binary);
    if (outstream_binary.fail()) throw std::runtime_error("failed to open " + tempFilename);

    PlyFile tempFile;

    tempFile.add_properties_to_element("vertex", { "x", "y", "z" },
                                        Type::FLOAT32, cube.vertices.size(), reinterpret_cast<uint8_t*>(vertices.data()), Type::INVALID, 0);

    tempFile.add_properties_to_element("vertex", { "r", "g", "b" },
                                        Type::FLOAT32, cube.normals.size(), reinterpret_cast<uint8_t*>(cube.normals.data()), Type::INVALID, 0);

    tempFile.add_properties_to_element("face", { "vertex_indices" },
                                        Type::UINT32, cube.triangles.size(), reinterpret_cast<uint8_t*>(cube.triangles.data()), Type::UINT8, 3);

    tempFile.get_comments().push_back("generated by tinyply 2.2");

    // Write a binary file
    tempFile.write(outstream_binary, true);
}